...
One `AskUserQuestion` round, then proceed. Ask only what changes the work: topic boundaries and what
is explicitly out of scope, time window (default: last 5 years, weighted to the last 2), emphasis
(build oriented, academic, or commercial), and corpus size target (default 40 to 60 verified papers).
